...
$stAction = New-ScheduledTaskAction -Execute "C:\ProgramData\chocolatey\bin\SetDefaultBrowser.exe" -Argument 'Ede'
...
### Combine data disks into single volume, where all VMs will be created in ###

$PhysicalDisks = Get-StorageSubSystem -FriendlyName "Windows Storage*" | Get-PhysicalDisk -CanPool $True

Start-Job -Name CombineDisks -ScriptBlock { 
New-StoragePool -FriendlyName "CompanyData" -StorageSubsystemFriendlyName "Windows Storage*" -PhysicalDisks $PhysicalDisks ` 
| New-VirtualDisk -FriendlyName "AsHciDisk" -UseMaximumSize -ResiliencySettingName Simple `
| Initialize-Disk -PassThru | New-Partition -AssignDriveLetter -UseMaximumSize `
| Format-Volume -NewFileSystemLabel "AsHciData"
}

Wait-Job -Name CombineDisks




# Create folder for VHDs
New-Item -Path "D:\" -Name "VMs" -ItemType "directory"

Start-Sleep -Seconds 5

# Download WS 2019 file, run as job and wait till it finishes to continue 
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
$WSuri = "https://aka.ms/AAbclsv"
Start-Job -Name WebReq -ScriptBlock { Invoke-WebRequest -Uri $WSuri -OutFile "C:\VHDs\GUI.vhdx" }
